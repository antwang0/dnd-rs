...
impl ActorInstance {
    ...
    pub fn can_consume_resource(&self, resource: Resource) -> bool {
        // Stunned, Incapacitated, and Unconscious actors lose their entire
        // action economy. Prone / Restrained are NOT checked for Movement:
        // stand-up itself pays in Movement, so blocking the resource here
        // would create a catch-22. Move-the-action is still blocked because
        // `remaining_movement()` returns 0 when Prone/Restrained, which
        // makes `path_cost_to` find no path.
        let blocked = self.has_condition(Condition::Stunned)
