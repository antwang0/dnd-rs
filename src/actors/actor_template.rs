...
impl ActorInstance {
    ...
    pub fn can_consume_resource(&self, resource: Resource) -> bool {
        ...
        let action_blocked =
            self.has_condition(Condition::Stunned) || self.has_condition(Condition::Incapacitated);
        match resource {
    ...
    pub fn remaining_movement(&self) -> f32 {
        // 5e: each of these conditions independently sets speed to 0.
        // Listed verbatim so adding another condition doesn't accidentally
        // get wrapped into one big bool.
        let speed_zeroed = self.has_condition(Condition::Prone)
