...
impl ActorInstance {
    ...
    pub fn can_consume_resource(&self, resource: Resource) -> bool {
        // Action-locking conditions (Stunned today) clear Action / Bonus /
        // Reaction / SpellSlot / Legendary. Prone is NOT checked here for
        // Movement: stand-up itself pays in Movement, so blocking the
        // resource here would create a catch-22. Move-the-action is still
        // blocked because `remaining_movement()` returns 0 when Prone, so
        // `path_cost_to` finds no path.
        let action_locked = self
            .conditions
            .keys()
            .any(|c| c.locks_actions());
        match resource {
