...
pub struct CreatureTemplate {
    ...
    /// Damage types this creature takes only half of (5e resistance).
    /// Applied before `take_damage` runs.
    pub resistances: HashSet<DamageType>,
    /// Damage types this creature ignores entirely (immunity).
    pub immunities: HashSet<DamageType>,
    /// Damage types this creature takes double of (vulnerability).
...
impl ActorInstance {
    ...
    /// Apply 5e resistance / immunity / vulnerability to an incoming
    /// damage value. Immunity wins over both, then vulnerability doubles,
    /// then resistance halves (rounding down per 5e). Returns the final
    /// HP delta to subtract.
    pub fn adjust_damage(&self, amount: u32, dt: DamageType) -> u32 {
    ...
    pub fn has_resistance(&self, dt: DamageType) -> bool {
        self.resistances.contains(&dt)
    }

    pub fn has_immunity(&self, dt: DamageType) -> bool {
        self.immunities.contains(&dt)
    }

    pub fn has_vulnerability(&self, dt: DamageType) -> bool {
    ...
    pub fn can_consume_resource(&self, resource: Resource) -> bool {
        // Action-economy-locking conditions (Stunned / Incapacitated /
        // Paralyzed) block actions, bonus actions, reactions, and any
        // action that pays a spell-slot cost. Movement is intentionally
        // *not* gated here for Prone, even though Prone immobilizes:
        // stand-up itself pays in Movement, so blocking it would create
        // a catch-22. `remaining_movement` returns 0 for the immobilizing
        // set, which already prevents `path_cost_to` from finding paths.
        let action_locked = self
            .conditions
            .keys()
            .any(|c| c.locks_action_economy());
        match resource {
            ...
            Resource::Action => !action_locked && self.action_slots >= 1,
            Resource::BonusAction => !action_locked && self.bonus_action_slots >= 1,
            Resource::Reaction => !action_locked && self.reaction_slots >= 1,
            Resource::LegendaryAction => !action_locked && self.legendary_action_slots >= 1,
        }
    }
    ...
    pub fn remaining_movement(&self) -> f32 {
        if self.conditions.keys().any(|c| c.immobilizes()) {
            return 0.0;
        }
        self.movement
    }
