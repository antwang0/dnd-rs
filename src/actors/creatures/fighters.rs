...
pub static CAVALIER_FIGHTER_TEMPLATE: LazyLock<CreatureTemplate> = LazyLock::new(|| {
    ...
    // its level-3 **Born to the Saddle** and level-10 **Hold the Line**
    // are both written on the assumption that the fighter is on a horse
    // — and the engine's builds are level-complete characters rather
    // than a class plus a feat budget. So the Cavalier ships with it the
    // way the Champion ships with a crit threshold of 19.
    //
    // It also gives the charge below somewhere to point. Ferocious
    // Charger wants twenty straight feet before the swing, and a fighter
