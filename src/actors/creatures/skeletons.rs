...
pub static SKELETON_TEMPLATE: LazyLock<CreatureTemplate> = LazyLock::new(|| {
    ...
    CreatureTemplate {
        ...
        // 5e Skeleton: vulnerable to bludgeoning (brittle bones);
        // immune to poison (no organs); undead necrotic resist.
        resistances: HashSet::from([DamageType::Necrotic]),
        immunities: HashSet::from([DamageType::Poison]),
        vulnerabilities: HashSet::from([DamageType::Bludgeoning, DamageType::Radiant]),
    }
});
