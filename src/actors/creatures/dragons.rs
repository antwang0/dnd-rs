...
struct DragonRow {
    ...
    speed: f32,
    fly_speed: f32,
    /// RAW's **Burrow** line, in feet, and `0.0` for the seven colours
    /// that do not dig.
    ///
    /// Three do: the blue, the brass and the white, at all four ages,
    /// and the magnitude is the age's rather than the colour's — 15,
    /// 20, 30, 40 from wyrmling to ancient, the same at each of the
    /// three. Stored per row anyway, beside the two speeds that are
    /// already columns, because the alternative is a rule in
    /// `dragon_template` that a reader would have to cross-check
    /// against twelve stat blocks to believe.
    ///
    /// See `crate::engine::burrowing` for what being under the floor
    /// costs and buys; the short version is that a blue dragon can now
    /// leave the room through its own ceiling.
    burrow: f32,
