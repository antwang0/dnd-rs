    ...
    vec![
            (
                ...
                vec![
                    ...
                    &*barbarians::GIANT_BARBARIAN_TEMPLATE,
                ],
            ),
            (
                "artificer",
...
mod tests {
    ...
    /// One row of SRD 5.2's stat-block header, for a template the book
    /// prints: the five numbers off the top of the page, plus the
    /// creature they belong to.
    ///
    /// Deliberately only the header. Traits, actions, senses and damage
    /// rows all have their own sweeps or their own reasons not to; what
    /// this row carries is the part of a stat block that is *pure data*
    /// and therefore drifts silently — a number that is one off is a
    /// fight that plays slightly wrong and nothing that ever fails.
    struct SrdStatBlock {
        template: &'static CreatureTemplate,
        ac: u32,
        /// The hit-dice expression exactly as the book prints it inside
        /// the parentheses, whitespace removed: "6d10+12", "1d4-1".
        hitpoints: &'static str,
        /// STR, DEX, CON, INT, WIS, CHA — the order the block prints
        /// them.
        abilities: [u32; 6],
        cr: f32,
    }

    /// Every template in the bestiary that SRD 5.2 also prints, with the
    /// book's own numbers beside it.
    ///
    /// Written down rather than derived, for the reason every table in
    /// this module is: the book is the authority and the book is not
    /// code. What makes this one worth its length is what it caught the
    /// day it was written — a hundred and fifty-six stat blocks whose
    /// headline numbers had drifted, most of them because they were
    /// entered from the 2014 printing and the bestiary has been moving
    /// to 5.2 one creature at a time ever since. An Ogre filed at CR 1
    /// with half the hit points the book gives it is not a balance
    /// choice; it is a hole in the encounter generator's budget, and
    /// nothing in the engine could have told anybody it was there.
    ///
    /// Creatures the book does not print are absent, and there are a
    /// lot of them: the beholder, the mind flayer and the displacer
    /// beast are WotC product identity rather than SRD content, the
    /// twelve player-class chassis are builds rather than stat blocks,
    /// and a handful (the Hobgoblin Warlord, the Fire Imp) are 2014
    ...
    fn srd_stat_block_table() -> Vec<SrdStatBlock> {
        use super::*;
        [
        (&*ankhegs::ANKHEG_TEMPLATE, 14, "6d10+12", [17, 11, 14, 1, 13, 6], 2.0),
        (&*animated_armors::ANIMATED_ARMOR_TEMPLATE, 18, "6d8+6", [14, 11, 13, 1, 3, 1], 1.0),
        (&*bandits::BANDIT_TEMPLATE, 12, "2d8+2", [11, 12, 12, 10, 10, 10], 0.125),
        (&*bandit_captains::BANDIT_CAPTAIN_TEMPLATE, 15, "8d8+16", [15, 16, 14, 14, 11, 14], 2.0),
        (&*basilisks::BASILISK_TEMPLATE, 15, "8d8+16", [16, 8, 15, 2, 8, 7], 3.0),
        (&*berserkers::BERSERKER_TEMPLATE, 13, "9d8+27", [16, 12, 17, 9, 11, 9], 2.0),
        (&*bugbears::BUGBEAR_TEMPLATE, 14, "6d8+6", [15, 14, 13, 8, 11, 9], 1.0),  // SRD 5.2: Bugbear Warrior
        (&*chimeras::CHIMERA_TEMPLATE, 14, "12d10+48", [19, 11, 19, 3, 14, 10], 6.0),
        (&*chuuls::CHUUL_TEMPLATE, 16, "9d10+27", [19, 10, 16, 5, 11, 5], 4.0),
        (&*cloakers::CLOAKER_TEMPLATE, 14, "14d10+14", [17, 15, 12, 13, 14, 7], 8.0),
        (&*cockatrices::COCKATRICE_TEMPLATE, 11, "5d6+5", [6, 12, 12, 2, 13, 5], 0.5),
        (&*dire_wolves::DIRE_WOLF_TEMPLATE, 14, "3d10+6", [17, 15, 15, 3, 12, 7], 1.0),
        (&*doppelgangers::DOPPELGANGER_TEMPLATE, 14, "8d8+16", [11, 18, 14, 11, 12, 14], 3.0),
        (&*ettins::ETTIN_TEMPLATE, 12, "10d10+30", [21, 8, 17, 6, 10, 8], 4.0),
        (&*fire_elementals::FIRE_ELEMENTAL_TEMPLATE, 13, "11d10+33", [10, 17, 16, 6, 10, 7], 5.0),
        (&*gargoyles::GARGOYLE_TEMPLATE, 15, "9d8+27", [15, 11, 16, 6, 11, 7], 2.0),
        (&*gelatinous_cubes::GELATINOUS_CUBE_TEMPLATE, 6, "6d10+30", [14, 3, 20, 1, 6, 1], 2.0),
        (&*ghosts::GHOST_TEMPLATE, 11, "10d8", [7, 13, 10, 10, 12, 17], 4.0),
        (&*ghouls::GHOUL_TEMPLATE, 12, "5d8", [13, 15, 10, 7, 10, 6], 1.0),
        (&*giant_scorpions::GIANT_SCORPION_TEMPLATE, 15, "7d10+14", [16, 13, 15, 1, 9, 3], 3.0),
        (&*gnolls::GNOLL_TEMPLATE, 15, "6d8", [14, 12, 11, 6, 10, 7], 0.5),  // SRD 5.2: Gnoll Warrior
        (&*gricks::GRICK_TEMPLATE, 14, "12d8", [14, 14, 11, 3, 14, 5], 2.0),
        (&*goblins::GOBLIN_TEMPLATE, 15, "3d6", [8, 15, 10, 10, 8, 8], 0.25),  // SRD 5.2: Goblin Warrior
        (&*goblin_bosses::GOBLIN_BOSS_TEMPLATE, 17, "6d6", [10, 15, 10, 10, 8, 10], 1.0),
        (&*harpies::HARPY_TEMPLATE, 11, "7d8+7", [12, 13, 12, 7, 10, 13], 1.0),
        (&*hell_hounds::HELL_HOUND_TEMPLATE, 15, "9d8+18", [17, 12, 14, 6, 13, 6], 3.0),
        (&*hill_giants::HILL_GIANT_TEMPLATE, 13, "10d12+40", [21, 8, 19, 5, 9, 6], 5.0),
        (&*hippogriffs::HIPPOGRIFF_TEMPLATE, 11, "4d10+4", [17, 13, 13, 2, 12, 8], 1.0),
        (&*hobgoblins::HOBGOBLIN_TEMPLATE, 18, "2d8+2", [13, 12, 12, 10, 10, 9], 0.5),  // SRD 5.2: Hobgoblin Warrior
        (&*hydras::HYDRA_TEMPLATE, 15, "16d12+80", [20, 12, 20, 2, 10, 7], 8.0),
        (&*kobolds::KOBOLD_TEMPLATE, 14, "3d6-3", [7, 15, 9, 8, 7, 8], 0.125),  // SRD 5.2: Kobold Warrior
        (&*knights::KNIGHT_TEMPLATE, 18, "8d8+16", [16, 11, 14, 11, 11, 15], 3.0),
        (&*mages::MAGE_TEMPLATE, 15, "18d8", [9, 14, 11, 17, 12, 11], 6.0),
        (&*manticores::MANTICORE_TEMPLATE, 14, "8d10+24", [17, 16, 17, 7, 12, 8], 3.0),
        (&*medusas::MEDUSA_TEMPLATE, 15, "17d8+51", [10, 17, 16, 12, 13, 15], 6.0),
        (&*mimics::MIMIC_TEMPLATE, 12, "9d8+18", [17, 12, 15, 5, 13, 8], 2.0),
        (&*minotaurs::MINOTAUR_TEMPLATE, 14, "10d10+30", [18, 11, 16, 6, 16, 9], 3.0),  // SRD 5.2: Minotaur of Baphomet
        (&*mummies::MUMMY_TEMPLATE, 11, "9d8+18", [16, 8, 15, 6, 12, 12], 3.0),
        (&*nightmares::NIGHTMARE_TEMPLATE, 13, "8d10+24", [18, 15, 16, 10, 13, 15], 3.0),
        (&*ogres::OGRE_TEMPLATE, 11, "8d10+24", [19, 8, 16, 5, 7, 7], 2.0),
        (&*owlbears::OWLBEAR_TEMPLATE, 13, "7d10+21", [20, 12, 17, 3, 12, 7], 3.0),
        (&*phase_spiders::PHASE_SPIDER_TEMPLATE, 14, "7d10+7", [15, 16, 12, 6, 10, 6], 3.0),
        (&*salamanders::SALAMANDER_TEMPLATE, 15, "12d10+24", [18, 14, 15, 11, 10, 12], 5.0),
        (&*ropers::ROPER_TEMPLATE, 20, "11d10+33", [18, 8, 17, 7, 16, 6], 5.0),
        (&*rust_monsters::RUST_MONSTER_TEMPLATE, 14, "6d8+6", [13, 12, 13, 2, 13, 6], 0.5),
        (&*shadows::SHADOW_TEMPLATE, 12, "5d8+5", [6, 14, 13, 6, 10, 8], 0.5),
        (&*shambling_mounds::SHAMBLING_MOUND_TEMPLATE, 15, "13d10+39", [18, 8, 16, 5, 10, 5], 5.0),
        (&*specters::SPECTER_TEMPLATE, 12, "5d8", [1, 14, 11, 10, 10, 11], 1.0),
        (&*giant_spiders::GIANT_SPIDER_TEMPLATE, 14, "4d10+4", [14, 16, 12, 2, 11, 4], 1.0),
        (&*stirges::STIRGE_TEMPLATE, 13, "2d4", [4, 16, 11, 2, 8, 6], 0.125),
        (&*stone_giants::STONE_GIANT_TEMPLATE, 17, "11d12+55", [23, 15, 20, 10, 12, 9], 7.0),
        (&*storm_giants::STORM_GIANT_TEMPLATE, 16, "20d12+100", [29, 14, 20, 16, 20, 18], 13.0),
        (&*treants::TREANT_TEMPLATE, 16, "12d12+60", [23, 8, 21, 12, 16, 12], 9.0),
        (&*trolls::TROLL_TEMPLATE, 15, "9d10+45", [18, 13, 20, 7, 9, 7], 5.0),
        (&*vampire_spawns::VAMPIRE_SPAWN_TEMPLATE, 16, "12d8+36", [16, 16, 16, 11, 10, 12], 5.0),
        (&*vrocks::VROCK_TEMPLATE, 15, "16d10+64", [17, 15, 18, 8, 13, 8], 6.0),
        (&*werewolves::WEREWOLF_TEMPLATE, 15, "11d8+22", [16, 14, 14, 10, 11, 10], 3.0),
        (&*wights::WIGHT_TEMPLATE, 14, "11d8+33", [15, 14, 16, 10, 13, 15], 3.0),
        (&*wisps::WISP_TEMPLATE, 19, "11d4", [1, 28, 10, 13, 14, 11], 2.0),  // SRD 5.2: Will-o’-Wisp
        (&*wolves::WOLF_TEMPLATE, 12, "2d8+2", [14, 15, 12, 3, 12, 6], 0.25),
        (&*worgs::WORG_TEMPLATE, 13, "4d10+4", [16, 13, 13, 7, 11, 8], 0.5),
        (&*wyverns::WYVERN_TEMPLATE, 14, "15d10+45", [19, 10, 16, 5, 12, 6], 6.0),
        (&*bone_devils::BONE_DEVIL_TEMPLATE, 16, "17d10+68", [18, 16, 18, 13, 14, 16], 9.0),
        (&*erinyes::ERINYES_TEMPLATE, 18, "21d8+84", [18, 16, 18, 14, 14, 18], 12.0),
        (&*frost_giants::FROST_GIANT_TEMPLATE, 15, "13d12+65", [23, 9, 21, 9, 10, 12], 8.0),
        (&*earth_elementals::EARTH_ELEMENTAL_TEMPLATE, 17, "14d10+70", [20, 8, 20, 5, 10, 5], 5.0),
        (&*air_elementals::AIR_ELEMENTAL_TEMPLATE, 15, "12d10+24", [14, 20, 14, 6, 10, 6], 5.0),
        (&*bulettes::BULETTE_TEMPLATE, 17, "9d10+45", [19, 11, 21, 2, 10, 5], 5.0),
        (&*wraiths::WRAITH_TEMPLATE, 13, "9d8+27", [6, 16, 16, 12, 14, 15], 5.0),
        (&*vampires::VAMPIRE_TEMPLATE, 16, "23d8+92", [18, 18, 18, 17, 15, 18], 13.0),
        (&*skeletons::SKELETON_TEMPLATE, 14, "2d8+4", [10, 16, 15, 6, 8, 5], 0.25),
        (&*zombies::ZOMBIE_TEMPLATE, 8, "2d8+6", [13, 6, 16, 3, 6, 5], 0.25),
        (&*imps::IMP_TEMPLATE, 13, "6d4+6", [6, 17, 13, 11, 12, 14], 1.0),
        (&*couatls::COUATL_TEMPLATE, 19, "8d8+24", [16, 20, 17, 18, 20, 18], 4.0),
        (&*giant_eagles::GIANT_EAGLE_TEMPLATE, 13, "4d10+4", [16, 17, 13, 8, 14, 10], 1.0),
        (&*sahuagins::SAHUAGIN_TEMPLATE, 12, "4d8+4", [13, 11, 12, 12, 13, 9], 0.5),  // SRD 5.2: Sahuagin Warrior
        (&*giant_apes::GIANT_APE_TEMPLATE, 12, "16d12+64", [23, 14, 18, 5, 12, 7], 7.0),
        (&*centaurs::CENTAUR_TEMPLATE, 16, "6d10+12", [18, 14, 14, 9, 13, 11], 2.0),  // SRD 5.2: Centaur Trooper
        (&*boars::BOAR_TEMPLATE, 11, "2d8+4", [13, 11, 14, 2, 9, 5], 0.25),
        (&*brown_bears::BROWN_BEAR_TEMPLATE, 11, "3d10+6", [17, 12, 15, 2, 13, 7], 1.0),
        (&*tigers::TIGER_TEMPLATE, 13, "4d10+8", [17, 16, 14, 3, 12, 8], 1.0),
        (&*giant_toads::GIANT_TOAD_TEMPLATE, 11, "6d10+6", [15, 13, 13, 2, 10, 3], 1.0),
        (&*pseudodragons::PSEUDODRAGON_TEMPLATE, 14, "3d4+3", [6, 15, 13, 10, 12, 10], 0.25),
        (&*behirs::BEHIR_TEMPLATE, 17, "16d12+64", [23, 16, 18, 7, 14, 12], 11.0),
        (&*polar_bears::POLAR_BEAR_TEMPLATE, 12, "5d10+15", [20, 14, 16, 2, 13, 7], 2.0),
        (&*lions::LION_TEMPLATE, 12, "4d10", [17, 15, 11, 3, 12, 8], 1.0),
        (&*fire_giants::FIRE_GIANT_TEMPLATE, 18, "13d12+78", [25, 9, 23, 10, 14, 13], 9.0),
        (&*rocs::ROC_TEMPLATE, 15, "16d20+80", [28, 10, 20, 3, 10, 9], 11.0),
        (&*pegasi::PEGASUS_TEMPLATE, 12, "7d10+21", [18, 15, 16, 10, 15, 13], 2.0),
        (&*winter_wolves::WINTER_WOLF_TEMPLATE, 13, "10d10+20", [18, 13, 14, 7, 12, 8], 3.0),
        (&*triceratopses::TRICERATOPS_TEMPLATE, 14, "12d12+36", [22, 9, 17, 2, 11, 5], 5.0),
        (&*tyrannosauruses::T_REX_TEMPLATE, 13, "13d12+52", [25, 10, 19, 2, 12, 9], 8.0),
        (&*water_elementals::WATER_ELEMENTAL_TEMPLATE, 14, "12d10+48", [18, 14, 18, 5, 10, 8], 5.0),
        (&*saber_toothed_tigers::SABER_TOOTHED_TIGER_TEMPLATE, 13, "7d10+14", [18, 17, 15, 3, 12, 8], 2.0),
        (&*hyenas::HYENA_TEMPLATE, 11, "1d8+1", [11, 13, 12, 2, 12, 5], 0.0),
        (&*giant_hyenas::GIANT_HYENA_TEMPLATE, 12, "6d10+12", [16, 14, 14, 2, 12, 7], 1.0),
        (&*green_hags::GREEN_HAG_TEMPLATE, 17, "11d8+33", [18, 12, 16, 13, 14, 14], 3.0),
        (&*gorgons::GORGON_TEMPLATE, 19, "12d10+48", [20, 11, 18, 2, 12, 7], 5.0),
        (&*dryads::DRYAD_TEMPLATE, 16, "5d8", [10, 12, 11, 14, 15, 18], 1.0),
        (&*quasits::QUASIT_TEMPLATE, 13, "10d4", [5, 17, 10, 7, 10, 10], 1.0),
        (&*succubi::SUCCUBUS_TEMPLATE, 15, "12d8+12", [8, 17, 13, 15, 12, 20], 4.0),  // SRD 5.2: Incubus
        (&*xorns::XORN_TEMPLATE, 19, "8d8+48", [17, 10, 22, 11, 10, 11], 5.0),
        (&*oni::ONI_TEMPLATE, 17, "14d10+42", [19, 11, 16, 14, 12, 15], 7.0),
        (&*merrow::MERROW_TEMPLATE, 13, "6d10+12", [18, 15, 15, 8, 10, 9], 2.0),
        (&*giant_crabs::GIANT_CRAB_TEMPLATE, 15, "3d8", [13, 13, 11, 1, 9, 3], 0.125),
        (&*cloud_giants::CLOUD_GIANT_TEMPLATE, 14, "16d12+96", [27, 10, 22, 12, 16, 16], 9.0),
        (&*hezrous::HEZROU_TEMPLATE, 18, "15d10+75", [19, 17, 20, 5, 12, 13], 8.0),
        (&*gibbering_mouthers::GIBBERING_MOUTHER_TEMPLATE, 9, "7d8+21", [10, 8, 16, 3, 10, 6], 2.0),
        (&*mummy_lords::MUMMY_LORD_TEMPLATE, 17, "25d8+75", [18, 10, 17, 11, 19, 16], 15.0),
        (&*iron_golems::IRON_GOLEM_TEMPLATE, 20, "24d10+120", [24, 9, 20, 3, 11, 1], 16.0),
        (&*rakshasas::RAKSHASA_TEMPLATE, 17, "26d8+104", [14, 17, 18, 13, 16, 20], 13.0),
        (&*dragon_turtles::DRAGON_TURTLE_TEMPLATE, 20, "23d20+115", [25, 10, 20, 10, 12, 12], 17.0),
        (&*krakens::KRAKEN_TEMPLATE, 18, "26d20+208", [30, 11, 26, 22, 18, 20], 23.0),
        (&*androsphinxes::ANDROSPHINX_TEMPLATE, 17, "19d10+95", [22, 10, 20, 16, 23, 18], 17.0),  // SRD 5.2: Sphinx of Valor
        (&*unicorns::UNICORN_TEMPLATE, 12, "13d10+26", [18, 14, 15, 11, 17, 16], 5.0),
        (&*driders::DRIDER_TEMPLATE, 19, "13d10+52", [16, 19, 18, 13, 16, 12], 6.0),
        (&*sea_hags::SEA_HAG_TEMPLATE, 14, "7d8+21", [16, 13, 16, 12, 12, 13], 2.0),
        (&*night_hags::NIGHT_HAG_TEMPLATE, 17, "15d8+45", [18, 15, 16, 16, 14, 16], 5.0),
        (&*spirit_nagas::SPIRIT_NAGA_TEMPLATE, 17, "18d10+36", [18, 17, 14, 16, 15, 16], 8.0),
        (&*otyughs::OTYUGH_TEMPLATE, 14, "11d10+44", [16, 11, 19, 6, 13, 6], 5.0),
        (&*sprites::SPRITE_TEMPLATE, 15, "4d4", [3, 18, 10, 14, 13, 11], 0.25),
        (&*death_dogs::DEATH_DOG_TEMPLATE, 12, "6d8+12", [15, 14, 14, 3, 13, 6], 1.0),
        (&*magmins::MAGMIN_TEMPLATE, 14, "3d6+3", [7, 15, 12, 8, 11, 10], 0.5),
        (&*griffons::GRIFFON_TEMPLATE, 12, "7d10+21", [18, 15, 16, 2, 13, 8], 2.0),
        (&*lamias::LAMIA_TEMPLATE, 13, "13d10+26", [16, 13, 15, 14, 15, 16], 4.0),
        (&*werebears::WEREBEAR_TEMPLATE, 15, "18d8+54", [19, 10, 17, 11, 12, 12], 5.0),
        (&*wereboars::WEREBOAR_TEMPLATE, 15, "15d8+30", [17, 10, 15, 10, 11, 8], 4.0),
        (&*wererats::WERERAT_TEMPLATE, 13, "11d8+11", [10, 16, 12, 11, 10, 8], 2.0),
        (&*weretigers::WERETIGER_TEMPLATE, 12, "16d8+48", [17, 15, 16, 10, 13, 11], 4.0),
        (&*ettercaps::ETTERCAP_TEMPLATE, 13, "8d8+8", [14, 15, 13, 7, 12, 8], 2.0),
        (&*awakened_trees::AWAKENED_TREE_TEMPLATE, 13, "7d12+14", [19, 6, 15, 10, 10, 7], 2.0),
        (&*dretches::DRETCH_TEMPLATE, 11, "4d6+4", [12, 11, 12, 5, 8, 3], 0.25),
        (&*lemures::LEMURE_TEMPLATE, 9, "2d8", [10, 5, 11, 1, 11, 3], 0.0),
        (&*bearded_devils::BEARDED_DEVIL_TEMPLATE, 13, "9d8+18", [16, 15, 15, 9, 11, 14], 3.0),
        (&*blink_dogs::BLINK_DOG_TEMPLATE, 13, "4d8+4", [12, 17, 12, 10, 13, 11], 0.25),
        (&*mephits::ICE_MEPHIT_TEMPLATE, 11, "6d6", [7, 13, 10, 9, 11, 12], 0.5),
        (&*mephits::STEAM_MEPHIT_TEMPLATE, 10, "5d6", [5, 11, 10, 11, 10, 12], 0.25),
        (&*mephits::MAGMA_MEPHIT_TEMPLATE, 11, "4d6+4", [8, 12, 12, 7, 10, 10], 0.5),
        (&*mephits::DUST_MEPHIT_TEMPLATE, 12, "5d6", [5, 14, 10, 9, 11, 10], 0.5),
        (&*black_puddings::BLACK_PUDDING_TEMPLATE, 7, "8d10+24", [16, 5, 16, 1, 6, 1], 4.0),
        (&*flesh_golems::FLESH_GOLEM_TEMPLATE, 9, "15d8+60", [19, 9, 18, 6, 10, 5], 5.0),
        (&*horned_devils::HORNED_DEVIL_TEMPLATE, 18, "19d10+95", [22, 17, 21, 12, 16, 18], 11.0),
        (&*nalfeshnees::NALFESHNEE_TEMPLATE, 18, "16d10+96", [21, 10, 22, 19, 12, 15], 13.0),
        (&*djinn::DJINNI_TEMPLATE, 17, "19d10+114", [21, 15, 22, 15, 16, 20], 11.0),
        (&*efreeti::EFREETI_TEMPLATE, 17, "17d10+119", [22, 12, 24, 16, 15, 19], 11.0),
        (&*constrictor_snakes::CONSTRICTOR_SNAKE_TEMPLATE, 13, "2d10+2", [15, 14, 12, 1, 10, 3], 0.25),
        (&*constrictor_snakes::GIANT_CONSTRICTOR_SNAKE_TEMPLATE, 12, "8d12+8", [19, 14, 12, 1, 10, 3], 2.0),
        (&*crocodiles::CROCODILE_TEMPLATE, 12, "2d10+2", [15, 10, 13, 2, 10, 5], 0.5),
        (&*crocodiles::GIANT_CROCODILE_TEMPLATE, 14, "9d12+27", [21, 9, 17, 2, 10, 7], 5.0),
        (&*invisible_stalkers::INVISIBLE_STALKER_TEMPLATE, 14, "13d10+26", [16, 19, 14, 10, 15, 11], 6.0),
        (&*purple_worms::PURPLE_WORM_TEMPLATE, 18, "15d20+90", [28, 7, 22, 1, 8, 4], 15.0),
        (&*mammoths::MAMMOTH_TEMPLATE, 13, "11d12+55", [24, 9, 21, 3, 11, 6], 6.0),
        (&*giant_octopuses::GIANT_OCTOPUS_TEMPLATE, 11, "7d10+7", [17, 13, 13, 5, 10, 4], 1.0),
        (&*plesiosauruses::PLESIOSAURUS_TEMPLATE, 13, "8d10+24", [18, 15, 16, 2, 12, 5], 2.0),
        (&*pteranodons::PTERANODON_TEMPLATE, 13, "3d8", [12, 15, 10, 2, 9, 5], 0.25),
        (&*scouts::SCOUT_TEMPLATE, 13, "3d8+3", [11, 14, 12, 11, 13, 11], 0.5),
        (&*giant_rats::GIANT_RAT_TEMPLATE, 13, "2d6", [7, 16, 11, 2, 10, 4], 0.125),
        (&*ghasts::GHAST_TEMPLATE, 13, "8d8", [16, 17, 10, 11, 10, 8], 2.0),
        (&*commoners::COMMONER_TEMPLATE, 10, "1d8", [10, 10, 10, 10, 10, 10], 0.0),
        (&*mastiffs::MASTIFF_TEMPLATE, 12, "1d8+1", [13, 14, 12, 3, 12, 7], 0.125),
        (&*guards::GUARD_TEMPLATE, 16, "2d8+2", [13, 12, 12, 10, 11, 10], 0.125),
        (&*grimlocks::GRIMLOCK_TEMPLATE, 11, "2d8+2", [16, 12, 12, 9, 8, 6], 0.25),
        (&*giant_frogs::GIANT_FROG_TEMPLATE, 11, "4d8", [12, 13, 11, 2, 10, 3], 0.25),
        (&*hawks::HAWK_TEMPLATE, 13, "1d4-1", [5, 16, 8, 2, 14, 6], 0.0),
        (&*giant_lizards::GIANT_LIZARD_TEMPLATE, 12, "3d10+3", [15, 12, 13, 2, 10, 5], 0.25),
        (&*giant_wolf_spiders::GIANT_WOLF_SPIDER_TEMPLATE, 13, "2d8+2", [12, 16, 13, 3, 12, 4], 0.25),
        (&*reef_sharks::REEF_SHARK_TEMPLATE, 12, "4d8+4", [14, 15, 13, 1, 10, 4], 0.5),
        (&*hunter_sharks::HUNTER_SHARK_TEMPLATE, 12, "6d10+12", [18, 14, 15, 1, 10, 4], 2.0),
        (&*giant_sharks::GIANT_SHARK_TEMPLATE, 13, "8d12+40", [23, 11, 21, 1, 10, 5], 5.0),
        (&*warhorses::WARHORSE_TEMPLATE, 11, "3d10+3", [18, 12, 13, 2, 12, 7], 0.5),
        (&*giant_vultures::GIANT_VULTURE_TEMPLATE, 10, "3d10+9", [15, 10, 16, 6, 12, 7], 1.0),
        (&*giant_bats::GIANT_BAT_TEMPLATE, 13, "4d10", [15, 16, 11, 2, 12, 6], 0.25),
        (&*giant_centipedes::GIANT_CENTIPEDE_TEMPLATE, 14, "2d6+2", [5, 14, 12, 1, 7, 3], 0.25),
        (&*giant_boars::GIANT_BOAR_TEMPLATE, 13, "5d10+15", [17, 10, 16, 2, 7, 5], 2.0),
        (&*giant_goats::GIANT_GOAT_TEMPLATE, 11, "3d10+3", [17, 13, 12, 3, 12, 6], 0.5),
        (&*giant_owls::GIANT_OWL_TEMPLATE, 12, "3d10+3", [13, 15, 12, 10, 14, 10], 0.25),
        (&*giant_venomous_snakes::GIANT_VENOMOUS_SNAKE_TEMPLATE, 14, "2d8+2", [10, 18, 13, 2, 10, 3], 0.25),
        (&*killer_whales::KILLER_WHALE_TEMPLATE, 12, "12d12+12", [19, 14, 13, 3, 12, 7], 3.0),
        (&*riding_horses::RIDING_HORSE_TEMPLATE, 11, "2d10+2", [16, 13, 12, 2, 11, 7], 0.25),
        (&*draft_horses::DRAFT_HORSE_TEMPLATE, 10, "2d10+4", [18, 10, 15, 2, 11, 7], 0.25),
        (&*awakened_shrubs::AWAKENED_SHRUB_TEMPLATE, 9, "3d6", [3, 8, 11, 10, 10, 6], 0.0),
        (&*bats::BAT_TEMPLATE, 12, "1d4-1", [2, 15, 8, 2, 12, 4], 0.0),
        (&*rats::RAT_TEMPLATE, 10, "1d4-1", [2, 11, 9, 2, 10, 4], 0.0),
        (&*camels::CAMEL_TEMPLATE, 10, "2d10+6", [15, 8, 17, 2, 11, 5], 0.125),
        (&*giant_badgers::GIANT_BADGER_TEMPLATE, 13, "2d8+6", [13, 10, 17, 2, 12, 5], 0.25),
        (&*giant_wasps::GIANT_WASP_TEMPLATE, 13, "5d8", [10, 14, 10, 1, 10, 3], 0.5),
        (&*cats::CAT_TEMPLATE, 12, "1d4", [3, 15, 10, 3, 12, 7], 0.0),
        (&*frogs::FROG_TEMPLATE, 11, "1d4-1", [1, 13, 8, 1, 8, 3], 0.0),
        (&*lizards::LIZARD_TEMPLATE, 10, "1d4", [2, 11, 10, 1, 8, 3], 0.0),
        (&*weasels::WEASEL_TEMPLATE, 13, "1d4-1", [3, 16, 8, 2, 12, 3], 0.0),
        (&*goats::GOAT_TEMPLATE, 10, "1d8", [11, 10, 11, 2, 10, 5], 0.0),
        (&*mules::MULE_TEMPLATE, 10, "2d8+2", [14, 10, 13, 2, 10, 5], 0.125),
        (&*ponies::PONY_TEMPLATE, 10, "2d8+2", [15, 10, 13, 2, 11, 7], 0.125),
        (&*elks::ELK_TEMPLATE, 10, "2d10", [16, 10, 11, 2, 10, 6], 0.25),
        (&*aboleths::ABOLETH_TEMPLATE, 17, "20d10+40", [21, 9, 15, 18, 15, 18], 10.0),
        (&*balors::BALOR_TEMPLATE, 19, "23d12+138", [26, 15, 22, 20, 16, 22], 19.0),
        (&*devas::DEVA_TEMPLATE, 17, "27d8+108", [18, 18, 18, 17, 20, 20], 10.0),
        (&*glabrezus::GLABREZU_TEMPLATE, 17, "18d10+90", [20, 15, 21, 19, 17, 16], 9.0),
        (&*liches::LICH_TEMPLATE, 20, "42d8+126", [11, 16, 16, 21, 14, 16], 21.0),
        (&*mariliths::MARILITH_TEMPLATE, 16, "21d10+105", [18, 20, 20, 18, 16, 20], 16.0),
        (&*pit_fiends::PIT_FIEND_TEMPLATE, 21, "27d10+189", [26, 14, 24, 22, 18, 24], 20.0),
        (&*solars::SOLAR_TEMPLATE, 21, "22d10+176", [26, 22, 26, 25, 25, 30], 21.0),
        (&*stone_golems::STONE_GOLEM_TEMPLATE, 18, "21d10+105", [22, 9, 20, 3, 11, 1], 10.0),
        (&*clay_golems::CLAY_GOLEM_TEMPLATE, 14, "13d10+52", [20, 9, 18, 3, 8, 1], 9.0),
        (&*tarrasques::TARRASQUE_TEMPLATE, 25, "34d20+340", [30, 11, 30, 3, 11, 11], 30.0),
        (&*cultists::CULTIST_TEMPLATE, 12, "2d8", [11, 12, 10, 10, 11, 10], 0.125),
        (&*nobles::NOBLE_TEMPLATE, 15, "2d8", [11, 12, 11, 12, 14, 16], 0.125),
        (&*spies::SPY_TEMPLATE, 12, "6d8", [10, 15, 10, 12, 14, 16], 1.0),
        (&*priests::PRIEST_TEMPLATE, 13, "7d8+7", [16, 10, 12, 13, 16, 13], 2.0),
        (&*gladiators::GLADIATOR_TEMPLATE, 16, "15d8+45", [18, 15, 16, 10, 12, 15], 5.0),
        (&*assassins::ASSASSIN_TEMPLATE, 16, "15d8+30", [11, 18, 14, 16, 11, 10], 8.0),
        (&*archmages::ARCHMAGE_TEMPLATE, 17, "31d8+31", [10, 14, 12, 20, 15, 16], 12.0),
        (&*azers::AZER_TEMPLATE, 17, "6d8+12", [17, 12, 15, 12, 13, 10], 2.0),  // SRD 5.2: Azer Sentinel
        (&*barbed_devils::BARBED_DEVIL_TEMPLATE, 15, "13d8+52", [16, 17, 18, 12, 14, 14], 5.0),
        (&*chain_devils::CHAIN_DEVIL_TEMPLATE, 15, "10d8+40", [18, 15, 18, 11, 12, 14], 8.0),
        (&*darkmantles::DARKMANTLE_TEMPLATE, 11, "5d6+5", [16, 12, 13, 2, 10, 5], 0.5),
        (&*ochre_jellies::OCHRE_JELLY_TEMPLATE, 8, "7d10+14", [15, 6, 14, 2, 6, 1], 2.0),
        (&*satyrs::SATYR_TEMPLATE, 13, "7d8", [12, 16, 11, 12, 10, 14], 0.5),
        (&*shield_guardians::SHIELD_GUARDIAN_TEMPLATE, 17, "15d10+60", [18, 8, 18, 7, 10, 3], 7.0),
        (&*violet_fungi::VIOLET_FUNGUS_TEMPLATE, 5, "4d8", [3, 1, 10, 1, 3, 1], 0.25),
        (&*warhorse_skeletons::WARHORSE_SKELETON_TEMPLATE, 13, "3d10+6", [18, 12, 15, 2, 8, 5], 0.5),
        (&*panthers::PANTHER_TEMPLATE, 13, "3d8", [14, 16, 10, 3, 14, 7], 0.25),
        (&*remorhazes::REMORHAZ_TEMPLATE, 17, "17d12+85", [24, 13, 21, 4, 10, 5], 11.0),
        (&*rugs_of_smothering::RUG_OF_SMOTHERING_TEMPLATE, 12, "5d10", [17, 14, 10, 1, 3, 1], 2.0),  // SRD 5.2: Animated Rug of Smothering
        (&*merfolk::MERFOLK_TEMPLATE, 11, "2d8+2", [10, 13, 12, 11, 14, 12], 0.125),  // SRD 5.2: Merfolk Skirmisher
        (&*homunculi::HOMUNCULUS_TEMPLATE, 13, "1d4+2", [4, 15, 14, 10, 10, 7], 0.0),
        (&*giant_fire_beetles::GIANT_FIRE_BEETLE_TEMPLATE, 13, "1d6+1", [8, 10, 12, 1, 7, 3], 0.0),
        (&*giant_weasels::GIANT_WEASEL_TEMPLATE, 13, "2d8", [11, 17, 10, 4, 12, 5], 0.125),
        (&*jackals::JACKAL_TEMPLATE, 12, "1d6", [8, 15, 11, 3, 12, 6], 0.0),
        (&*ravens::RAVEN_TEMPLATE, 12, "1d4", [2, 14, 10, 5, 13, 6], 0.0),
        (&*vultures::VULTURE_TEMPLATE, 10, "1d8+1", [7, 10, 13, 2, 12, 4], 0.0),
        (&*allosauruses::ALLOSAURUS_TEMPLATE, 13, "6d10+18", [19, 13, 17, 2, 12, 5], 2.0),
        (&*ankylosauruses::ANKYLOSAURUS_TEMPLATE, 15, "8d12+16", [19, 11, 15, 2, 12, 5], 3.0),
        (&*apes::APE_TEMPLATE, 12, "3d8+6", [16, 14, 14, 6, 12, 7], 0.5),
        (&*archelons::ARCHELON_TEMPLATE, 17, "12d12+12", [18, 16, 13, 4, 14, 6], 4.0),
        (&*axe_beaks::AXE_BEAK_TEMPLATE, 11, "3d10+3", [14, 12, 12, 2, 10, 5], 0.25),
        (&*baboons::BABOON_TEMPLATE, 12, "1d6", [8, 14, 11, 4, 12, 6], 0.0),
        (&*badgers::BADGER_TEMPLATE, 11, "1d4+3", [10, 11, 16, 2, 12, 5], 0.0),
        (&*black_bears::BLACK_BEAR_TEMPLATE, 11, "3d8+6", [15, 12, 14, 2, 12, 7], 0.5),
        (&*blood_hawks::BLOOD_HAWK_TEMPLATE, 12, "2d6", [6, 14, 10, 3, 14, 5], 0.125),
        (&*crabs::CRAB_TEMPLATE, 11, "1d4+1", [6, 11, 12, 1, 8, 2], 0.0),
        (&*deer::DEER_TEMPLATE, 13, "1d8", [11, 16, 11, 2, 14, 5], 0.0),
        (&*eagles::EAGLE_TEMPLATE, 12, "1d6+1", [6, 15, 12, 2, 14, 7], 0.0),
        (&*elephants::ELEPHANT_TEMPLATE, 12, "8d12+24", [22, 9, 17, 3, 11, 6], 4.0),
        (&*flying_snakes::FLYING_SNAKE_TEMPLATE, 14, "2d4", [4, 15, 11, 2, 12, 5], 0.125),
        (&*giant_elks::GIANT_ELK_TEMPLATE, 14, "5d12+10", [19, 18, 14, 7, 14, 10], 2.0),
        (&*seahorses::GIANT_SEAHORSE_TEMPLATE, 14, "3d10", [15, 12, 11, 2, 12, 5], 0.5),
        (&*hippopotamuses::HIPPOPOTAMUS_TEMPLATE, 14, "11d10+22", [21, 7, 15, 2, 12, 4], 4.0),
        (&*octopuses::OCTOPUS_TEMPLATE, 12, "1d6", [4, 15, 11, 3, 10, 4], 0.0),
        (&*owls::OWL_TEMPLATE, 11, "1d4-1", [3, 13, 8, 2, 12, 7], 0.0),
        (&*piranhas::PIRANHA_TEMPLATE, 13, "1d4-1", [2, 16, 9, 1, 7, 2], 0.0),
        (&*rhinoceroses::RHINOCEROS_TEMPLATE, 13, "6d10+12", [21, 8, 15, 2, 12, 6], 2.0),
        (&*scorpions::SCORPION_TEMPLATE, 11, "1d4-1", [2, 11, 8, 1, 8, 2], 0.0),
        (&*seahorses::SEAHORSE_TEMPLATE, 12, "1d4-1", [1, 12, 8, 1, 10, 2], 0.0),
        (&*venomous_snakes::VENOMOUS_SNAKE_TEMPLATE, 12, "2d4", [2, 15, 11, 1, 10, 3], 0.125),
        (&*ice_devils::ICE_DEVIL_TEMPLATE, 18, "24d10+96", [21, 14, 18, 18, 15, 18], 14.0),
        (&*planetars::PLANETAR_TEMPLATE, 19, "21d10+147", [24, 20, 24, 19, 22, 25], 16.0),
        (&*sphinxes_of_wonder::SPHINX_OF_WONDER_TEMPLATE, 13, "7d4+7", [6, 17, 13, 15, 12, 11], 1.0),
        (&*gray_oozes::GRAY_OOZE_TEMPLATE, 9, "3d8+9", [12, 6, 16, 1, 6, 2], 0.5),
        (&*swarms::SWARM_OF_BATS_TEMPLATE, 12, "2d10", [5, 15, 10, 2, 12, 4], 0.25),
        (&*swarms::SWARM_OF_RATS_TEMPLATE, 10, "4d8-4", [9, 11, 9, 2, 10, 3], 0.25),
        (&*swarms::SWARM_OF_INSECTS_TEMPLATE, 11, "3d8+6", [3, 13, 14, 1, 7, 1], 0.5),
        (&*swarms::SWARM_OF_PIRANHAS_TEMPLATE, 13, "8d8-8", [13, 16, 9, 1, 7, 2], 1.0),
        (&*swarms::SWARM_OF_VENOMOUS_SNAKES_TEMPLATE, 14, "8d8", [8, 18, 11, 1, 10, 3], 2.0),
        (&*dragons::BLACK_DRAGON_WYRMLING_TEMPLATE, 17, "6d8+6", [15, 14, 13, 10, 11, 13], 2.0),
        (&*dragons::YOUNG_BLACK_DRAGON_TEMPLATE, 18, "15d10+45", [19, 14, 17, 12, 11, 15], 7.0),
        (&*dragons::ADULT_BLACK_DRAGON_TEMPLATE, 19, "17d12+85", [23, 14, 21, 14, 13, 19], 14.0),
        (&*dragons::ANCIENT_BLACK_DRAGON_TEMPLATE, 22, "21d20+147", [27, 14, 25, 16, 15, 22], 21.0),
        (&*dragons::BLUE_DRAGON_WYRMLING_TEMPLATE, 17, "10d8+20", [17, 10, 15, 12, 11, 15], 3.0),
        (&*dragons::YOUNG_BLUE_DRAGON_TEMPLATE, 18, "16d10+64", [21, 10, 19, 14, 13, 17], 9.0),
        (&*dragons::ADULT_BLUE_DRAGON_TEMPLATE, 19, "17d12+102", [25, 10, 23, 16, 15, 20], 16.0),
        (&*dragons::ANCIENT_BLUE_DRAGON_TEMPLATE, 22, "26d20+208", [29, 10, 27, 18, 17, 25], 23.0),
        (&*dragons::GREEN_DRAGON_WYRMLING_TEMPLATE, 17, "7d8+7", [15, 12, 13, 14, 11, 13], 2.0),
        (&*dragons::YOUNG_GREEN_DRAGON_TEMPLATE, 18, "16d10+48", [19, 12, 17, 16, 13, 15], 8.0),
        (&*dragons::ADULT_GREEN_DRAGON_TEMPLATE, 19, "18d12+90", [23, 12, 21, 18, 15, 18], 15.0),
        (&*dragons::ANCIENT_GREEN_DRAGON_TEMPLATE, 21, "23d20+161", [27, 12, 25, 20, 17, 22], 22.0),
        (&*dragons::RED_DRAGON_WYRMLING_TEMPLATE, 17, "10d8+30", [19, 10, 17, 12, 11, 15], 4.0),
        (&*dragons::YOUNG_RED_DRAGON_TEMPLATE, 18, "17d10+85", [23, 10, 21, 14, 11, 19], 10.0),
        (&*dragons::ADULT_RED_DRAGON_TEMPLATE, 19, "19d12+133", [27, 10, 25, 16, 13, 23], 17.0),
        (&*dragons::ANCIENT_RED_DRAGON_TEMPLATE, 22, "26d20+234", [30, 10, 29, 18, 15, 27], 24.0),
        (&*dragons::WHITE_DRAGON_WYRMLING_TEMPLATE, 16, "5d8+10", [14, 10, 14, 5, 10, 11], 2.0),
        (&*dragons::YOUNG_WHITE_DRAGON_TEMPLATE, 17, "13d10+52", [18, 10, 18, 6, 11, 12], 6.0),
        (&*dragons::ADULT_WHITE_DRAGON_TEMPLATE, 18, "16d12+96", [22, 10, 22, 8, 12, 12], 13.0),
        (&*dragons::ANCIENT_WHITE_DRAGON_TEMPLATE, 20, "18d20+144", [26, 10, 26, 10, 13, 18], 20.0),
        (&*dragons::BRASS_DRAGON_WYRMLING_TEMPLATE, 15, "4d8+4", [15, 10, 13, 10, 11, 13], 1.0),
        (&*dragons::YOUNG_BRASS_DRAGON_TEMPLATE, 17, "13d10+39", [19, 10, 17, 12, 11, 15], 6.0),
        (&*dragons::ADULT_BRASS_DRAGON_TEMPLATE, 18, "15d12+75", [23, 10, 21, 14, 13, 17], 13.0),
        (&*dragons::ANCIENT_BRASS_DRAGON_TEMPLATE, 20, "19d20+133", [27, 10, 25, 16, 15, 22], 20.0),
        (&*dragons::BRONZE_DRAGON_WYRMLING_TEMPLATE, 15, "6d8+12", [17, 10, 15, 12, 11, 15], 2.0),
        (&*dragons::YOUNG_BRONZE_DRAGON_TEMPLATE, 17, "15d10+60", [21, 10, 19, 14, 13, 17], 8.0),
        (&*dragons::ADULT_BRONZE_DRAGON_TEMPLATE, 18, "17d12+102", [25, 10, 23, 16, 15, 20], 15.0),
        (&*dragons::ANCIENT_BRONZE_DRAGON_TEMPLATE, 22, "24d20+192", [29, 10, 27, 18, 17, 25], 22.0),
        (&*dragons::COPPER_DRAGON_WYRMLING_TEMPLATE, 16, "4d8+4", [15, 12, 13, 14, 11, 13], 1.0),
        (&*dragons::YOUNG_COPPER_DRAGON_TEMPLATE, 17, "14d10+42", [19, 12, 17, 16, 13, 15], 7.0),
        (&*dragons::ADULT_COPPER_DRAGON_TEMPLATE, 18, "16d12+80", [23, 12, 21, 18, 15, 18], 14.0),
        (&*dragons::ANCIENT_COPPER_DRAGON_TEMPLATE, 21, "21d20+147", [27, 12, 25, 20, 17, 22], 21.0),
        (&*dragons::GOLD_DRAGON_WYRMLING_TEMPLATE, 17, "8d8+24", [19, 14, 17, 14, 11, 16], 3.0),
        (&*dragons::YOUNG_GOLD_DRAGON_TEMPLATE, 18, "17d10+85", [23, 14, 21, 16, 13, 20], 10.0),
        (&*dragons::ADULT_GOLD_DRAGON_TEMPLATE, 19, "18d12+126", [27, 14, 25, 16, 15, 24], 17.0),
        (&*dragons::ANCIENT_GOLD_DRAGON_TEMPLATE, 22, "28d20+252", [30, 14, 29, 18, 17, 28], 24.0),
        (&*dragons::SILVER_DRAGON_WYRMLING_TEMPLATE, 17, "6d8+18", [19, 10, 17, 12, 11, 15], 2.0),
        (&*dragons::YOUNG_SILVER_DRAGON_TEMPLATE, 18, "16d10+80", [23, 10, 21, 14, 11, 19], 9.0),
        (&*dragons::ADULT_SILVER_DRAGON_TEMPLATE, 19, "16d12+112", [27, 10, 25, 16, 13, 22], 16.0),
        (&*dragons::ANCIENT_SILVER_DRAGON_TEMPLATE, 22, "24d20+216", [30, 10, 29, 18, 15, 26], 23.0),
        ]
        .into_iter()
        .map(|(template, ac, hitpoints, abilities, cr)| SrdStatBlock {
            template,
            ac,
            hitpoints,
            abilities,
    ...
    /// Every headline number in the bestiary agrees with the book.
    ///
    /// One sweep rather than five, and one failure message listing
    /// everything that disagrees rather than stopping at the first,
    /// because the failure this guards against arrives in batches: a
    /// stat block entered from the wrong printing is usually wrong in
    /// four fields at once, and being told about one of them at a time
    /// is four runs of the suite to learn one creature's worth of news.
    #[test]
    fn the_bestiary_agrees_with_the_srd_about_the_top_of_the_page() {
        let mut wrong: Vec<String> = Vec::new();
        for row in srd_stat_block_table() {
            let t = row.template;
            if t.ac != row.ac {
                wrong.push(format!("{}: AC {} should be {}", t.name, t.ac, row.ac));
            }
            let printed = t.hitpoints.to_string();
            if printed != row.hitpoints {
                wrong.push(format!(
                    "{}: hit dice {} should be {}",
                    t.name, printed, row.hitpoints
                ));
            }
            // In the order the book prints them, which is also the
            // order `SrdStatBlock::abilities` stores them.
            let have = [
                ("STR", t.strength),
                ("DEX", t.dexterity),
                ("CON", t.constitution),
            ...
            for (i, (label, score)) in have.iter().enumerate() {
                if *score != row.abilities[i] {
                    wrong.push(format!(
                        "{}: {} {} should be {}",
                        t.name, label, score, row.abilities[i]
                    ));
                }
            }
            if (t.cr - row.cr).abs() > f32::EPSILON {
                wrong.push(format!("{}: CR {} should be {}", t.name, t.cr, row.cr));
            }
        }
        assert!(
            wrong.is_empty(),
            "{} headline numbers disagree with SRD 5.2:\n  {}",
            wrong.len(),
            wrong.join("\n  ")
        );
    }
