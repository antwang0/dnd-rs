...
impl Controller for SimpleAi {
    fn decide(&self, encounter: &EncounterInstance, actor_id: usize) -> ControllerDecision {
        ...
        // 3. Self-heal: bonus-action / no-args heals (e.g. Second Wind)
        //    used by the actor on themselves when wounded. Runs before
        //    ally support so a low-HP fighter saves themselves.
        if let Some(aei) = try_self_heal(encounter, actor_id) {
            return ControllerDecision::Act(aei);
        }

        // 4. Heal a dying / wounded ally.
...
fn try_support_heal(
    ...
    // Healing-only — `is_heal` is the single source of truth for "this
    // restores HP", so buffs like Help and Dodge are correctly excluded.
...
mod tests {
    ...
    fn ai_picks_aoe_when_two_enemies_clustered() {
        ...
        let action = aei.action();
        // The cluster is two enemies; the AI should pick a Burst-schema
        // attack. Don't pin to a specific spell — multiple AoEs may apply.
        assert!(
            matches!(
                action.targeting_schema(),
                crate::actions::action_template::TargetingSchema::Burst { .. }
            ),
            "two-enemy cluster should pull AoE over single-target, got {}",
            action.name()
        );
    }
