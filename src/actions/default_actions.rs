...
/// Dodge action — defensive Action that grants the `Dodging` condition
/// for one round wrap. Until the timer expires, attacks against you have
/// disadvantage and you have advantage on DEX saves. 5e RAW says "until
/// the start of your next turn"; we approximate with `Rounds(1)`.
pub struct Dodge {}
...
/// opportunity attacks. We model this as a transient `Helped`-like
/// marker via a fresh `Disengaging` condition timer; the OA dispatcher
/// checks it before firing reactions.
...
impl Action for Disengage {
    ...
    fn side_effects(
        ...
        // The Disengage marker condition lives one round; the OA
        // dispatcher consults it before firing reactions.
        use crate::conditions::{Condition, ConditionTimer};
        use crate::engine::side_effects::ApplyCondition;
        vec![Box::new(ApplyCondition {
            actor_id: caster_id,
...
/// Shove action — STR (Athletics) check vs target's STR or DEX
/// (Athletics/Acrobatics, target picks). On success, target falls Prone.
/// Only usable in melee reach, only against creatures up to one size
/// larger. We collapse the contested check to a STR save vs DC
/// (8 + caster STR mod), which behaves like the contested check on
/// average. Costs an Action.
pub struct Shove {}
...
impl Action for Shove {
    ...
    fn custom_validate_input(
        ...
        // Forbid shoving creatures more than one size larger. Mapping the
        // 5e size ladder to integer ranks so the comparison is direct.
        ...
        size_rank(target.size()) <= size_rank(caster.size()) + 1
    }
    ...
    fn side_effects(
        ...
        let dc = 8 + modifier_from_score(caster.ability_score(AbilityScoreType::Strength));
        ...
        let target_name = encounter
            ...
            .unwrap_or_default();
        encounter.log(format!("{} shoves {}.", caster_name, target_name));
...
/// Integer rank for 5e size categories. Used by Shove to forbid pushing
/// creatures more than one size larger than the shover.
fn size_rank(s: crate::engine::types::Size) -> i32 {
