...
//! *Stench*, the Sea Hag's *Vile Appearance*. What differs between them
//! is the radius, the ability, the condition, who is eligible to be
//! caught at all, and whether making the save buys anything.
//!
...
//! ## The 24-hour clause
//!
//! Two of the three traits end with "Success: The target is immune to
//! this *X*'s *Trait* for 24 hours", and the Hezrou's does not. That one
...
//!
//!   - **Damage.** None of the three SRD start-of-turn emanations deals
//!     any; all three install a condition and nothing else. A future
...
//!   - **Emanations that catch allies.** RAW's wording is "any
//!     creature", and a hezrou's stench is as foul to the dretch beside
//!     it as to the paladin. The engine scopes them to hostiles, for
//!     the reason `AuraSide` exists at all: a monster whose signature
//!     trait poisons its own escort would be read as a bug by everyone
//!     who saw it, and no SRD encounter is built to exploit the
//!     difference. Named here rather than left as an accident.
