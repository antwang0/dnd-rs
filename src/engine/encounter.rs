...
impl EncounterInstance {
    ...
    /// 5e clauses we model:
    /// - Attacker Prone / Poisoned / Frightened / Restrained / Blinded → disadvantage.
    /// - Attacker Invisible → advantage.
    /// - Target Prone → melee attacks have advantage, ranged have disadvantage.
    /// - Target Stunned / Restrained / Blinded / Paralyzed-equiv → advantage.
    ...
    /// - `Poisoned` → disadvantage on saves (we conflate save vs check).
    /// - `Restrained` → disadvantage on DEX saves.
    /// - `Frightened` → disadvantage on all saves while frightened (we
    ///   simplify the "while you can see source" clause).
